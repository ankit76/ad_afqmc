...
def plot_density(coords, density, figname=None):
    density_tot = density[0] + density[1]
    density_spin = density[0] - density[1]
    dat = [*density, density_tot, density_spin]

    fig, axs = plt.subplots(1, 4, figsize=(12, 10), sharey=True)

    vmin_tot = numpy.amin(dat[:-1])
    vmax_tot = numpy.amax(dat[:-1])
    vmin_spin = numpy.amin(dat[-1])
    vmax_spin = numpy.amax(dat[-1])
...
def build_fe_single_occ_trial(mol, lattice, integrals, verbose=0):
    nup, ndown = mol.nelec
    n_sites = lattice.n_sites
    evals_h1, evecs_h1 = numpy.linalg.eigh(integrals["h1"])
    ...
    umf.mo_coeff = numpy.array([evecs_h1, evecs_h1])
...
def scan_ueff(iter, mol, lattice, dm_qmc, U=None, Ueff_prev=None, 
    ...
    for Ueff in Ueffs:
        if verbose: print(f'\n# Ueff = {Ueff}')
        integrals_mf = get_integrals(Ueff, lattice)
...
def check_convergence(dUeff, dE, ddelta_min, err_E, err_delta_min, 
                      tol_ueff=1e-3, tol_energy=1e-8, tol_delta_min=1e-5):
    abs_dE = abs(dE)
...
def run_scc(comm, U, nx, ny, n_elec, options, bc='open_x', run_cpmc=True, 
            proj_trs=False, npoint=10, max_iter=20, tmpdir='./', filename='scc.out', 
            approx_dm_pure=False, do_plot_density=False, save_dm_hf=True, 
            verbose=0):
    if bc == 'open_x': lattice = lattices.triangular_grid(nx, ny, open_x=True)
    else: lattice = lattices.triangular_grid(nx, ny)
    n_sites = lattice.n_sites
    filling = sum(n_elec) / (2*n_sites)
    if verbose: print(f'\n# Filling factor = {filling}')
    
    mol = build_mol(n_elec, verbose=verbose)
    integrals = get_integrals(U, lattice)
    ...
    if nx == 4:
        ...
        if verbose: print(f'\n# Using closed-shell free electron trial...')
    ...
    while (not conv) and (iter < max_iter):
        ...
        e_qmc, err_qmc = run_qmc(
                ...
                proj_trs=proj_trs, tmpdir=tmpdir, verbose=verbose)
        
        # Get rdm1.
        if comm.rank == 0: 
            dm_avg_qmc, dm_noise_qmc = load_dm_qmc(tmpdir=tmpdir)
        ...
        if do_plot_density and comm.rank == 0:
            ...
            density_qmc = numpy.array([numpy.diag(dm_s) for dm_s in dm_avg_qmc])
            plot_density(coords, density_qmc, f'{tmpdir}cpmc_density_iter={iter}.png')
        ...
        Ueffs, deltas, err_deltas, umfs, cost_func = scan_ueff(
                ...
                bounds_prev=bounds_prev, npoint=npoint, verbose=verbose)
        ...
        conv = check_convergence(dUeff, dE, ddelta_min, err_qmc_prev, err_delta_min_prev)
