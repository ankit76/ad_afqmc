...
parser.add_argument('--tol_delta_min', type=float, required=False, nargs='?', default=1e-4)
parser.add_argument('--tol_delta_qmc', type=float, required=False, nargs='?', default=1e-4)
...
tol_delta_qmc = args.tol_delta_qmc
...
# For saving.
tmpdir = f'/projects/bcdd/shufay/hubbard_tri/self_consistent_constraint/test_against_qsz/{trs_tag}/{dm_tag}/{nx}x{ny}/{bc}/U={U}/pin={pin_type}/{init_trial}_trial/conv_delta_qmc/'
...
conv = run_scc(
        ...
        filename=filename, approx_dm_pure=approx_dm_pure, 
        tol_delta_qmc=tol_delta_qmc, tol_delta_min=tol_delta_min,
