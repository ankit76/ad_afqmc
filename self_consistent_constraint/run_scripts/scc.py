...
def get_e_estimate(mol, lattice, integrals):
    ...
    gmf = scf.GHF(mol)
    gmf.get_hcore = lambda *args: scipy.linalg.block_diag(integrals["h1"], integrals["h1"])
...
def plot_density_slice(lattice, density, iy=1, figname=None, save=False):
    ...
    # Get slice along iy.
    site_nums = [ix * lattice.l_y + iy for ix in range(lattice.l_x)]
    slice_iy = coords[site_nums]
    x = slice_iy[:, 0]
    numpy.testing.assert_allclose(slice_iy[:, 1], iy)
...
def scan_ueff(iter, mol, lattice, dm_qmc, U=None, Ueff_prev=None, 
              ...
              verbose=False):
    """
    Scan Ueff values and determine the optimal Ueff that produces a UHF solution
    ...
    for Ueff in Ueffs:
        ...
        # RMS error between the UHF and CPMC rdm1s.
...
def run_scc(
        comm, U, nx, ny, n_elec, options, Ueff=None, bc='open_x', 
        run_cpmc=True, set_e_estimate=False, init_trial='fe_single_occ', 
        proj_trs=False, npoint=10, max_iter=20, tol_delta_min=1e-4,
    ...
    if init_trial == 'fe_double_occ':
        ...
        umf_opt = build_uhf_trial(mol, lattice, integrals_mf)
