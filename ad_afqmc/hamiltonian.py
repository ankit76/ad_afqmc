...
class hamiltonian():
  ...
  def prop_ham(self, ham, dt, _trial, wave_data=None):
    ...
    ham['mf_shifts_fp'] = ham['mf_shifts'] / 2. / self.nelec    
    ham['h0_prop'] = - ham['h0'] - jnp.sum(ham['mf_shifts']**2) / 2.
    ham['h0_prop_fp'] = [ (ham['h0_prop'] + ham['ene0']) / self.nelec, (ham['h0_prop'] + ham['ene0']) / self.nelec ]
...
class hamiltonian_uhf():
  ...
  def prop_ham(self, ham, dt, _trial, wave_data):
    ...
    ham['mf_shifts_fp'] = jnp.stack((ham['mf_shifts'] / self.nelec[0] / 2., ham['mf_shifts'] / self.nelec[1] / 2. ))
    ham['h0_prop'] = - ham['h0'] - jnp.sum(ham['mf_shifts']**2) / 2.
    ham['h0_prop_fp'] = jnp.stack(
        ((ham['h0_prop'] + ham['ene0']) / self.nelec[0] / 2., (ham['h0_prop'] + ham['ene0']) / self.nelec[1] / 2.))
...
class hamiltonian_noci():
  ...
  def rot_orbs_single_det(self, ham, trial_up, trial_dn):
    rot_h1 = [trial_up[:, :self.nelec[0]].T @ ham['h1'][0], trial_dn[:, :self.nelec[1]].T @ ham['h1'][1]]
    rot_chol = [jnp.einsum('pi,gij->gpj', trial_up[:, :self.nelec[0]].T, ham['chol'].reshape(-1, self.norb, self.norb)), jnp.einsum('pi,gij->gpj', trial_dn[:, :self.nelec[1]].T, ham['chol'].reshape(-1, self.norb, self.norb))]
  ...
  def rot_ham(self, ham, wave_data):
    ...
    ham['h1'] = ham['h1'].at[1].set((ham['h1'][1] + ham['h1'][1].T) / 2.)
    trial = wave_data[1]
    ham['rot_h1'], ham['rot_chol'] = vmap(self.rot_orbs_single_det, in_axes=(None, 0, 0))(ham, trial[0], trial[1])
    return ham
  ...
  def prop_ham(self, ham, dt, trial, wave_data):
    ...
    ham['mf_shifts'] = 1.j * \
        vmap(lambda x: jnp.sum(
            x.reshape(self.norb, self.norb) * dm))(ham['chol'])
    ham['mf_shifts_fp'] = jnp.stack((ham['mf_shifts'] / self.nelec[0] / 2., ham['mf_shifts'] / self.nelec[1] / 2. ))
    ham['h0_prop'] = - ham['h0'] - jnp.sum(ham['mf_shifts']**2) / 2.
    ham['h0_prop_fp'] = jnp.stack(((ham['h0_prop'] + ham['ene0']) / self.nelec[0] / 2., (ham['h0_prop'] + ham['ene0']) / self.nelec[1] / 2.))
    v0 = 0.5 * jnp.einsum('gik,gjk->ij', ham['chol'].reshape(-1, self.norb, self.norb),
                          ham['chol'].reshape(-1, self.norb, self.norb), optimize='optimal')
    v1 = jnp.real(1.j * jnp.einsum('g,gik->ik',
                                   ham['mf_shifts'], ham['chol'].reshape(-1, self.norb, self.norb)))
    ...
    ham['exp_h1'] = jnp.array(
        [jsp.linalg.expm(-dt * h1_mod[0] / 2.), jsp.linalg.expm(-dt * h1_mod[1] / 2.)])
    return ham
