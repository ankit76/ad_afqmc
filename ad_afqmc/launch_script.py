...
from ad_afqmc import config, propagation, wavefunctions
...
def read_fcidump(tmpdir=tmpdir):
...
def read_options(options, tmpdir=tmpdir):
    if options is None:
        try:
...
def read_observable(nmo, options, tmpdir=tmpdir):
    try:
...
def set_trial(options, mo_coeff, norb, nelec_sp, tmpdir=tmpdir):
    wave_data = {}
    try:
...
def _prep_afqmc(options=None):
    ...
    h0, h1, chol, ms, nelec, nmo, nchol, nelec_sp = read_fcidump()
    norb = nmo

    options = read_options(options)
    observable = read_observable(nmo, options)
    ...
    trial, wave_data = set_trial(options, mo_coeff, norb, nelec_sp)
