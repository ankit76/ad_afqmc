...
def unpackArray_s2_tr(walkerArray, nelec):
    return [walkerArray[:,:nelec[0]] + 1.j*walkerArray[:,nelec[0]:2*nelec[0]], 
            walkerArray[:,2*nelec[0]:2*nelec[0]+nelec[1]] + 1.j * walkerArray[:,2*nelec[0]+nelec[1]:]] 
...
def calc_energy_s2(walkerArray, nelec, ham_data, wave_data):
    ...
    def calc_energy(Atrial, Btrial, walker, ham_data):
        norb = Atrial.shape[0]
        h0, h1, chol = ham_data["h0"], ham_data["h1"][0], ham_data["chol"].reshape(-1,norb,norb)
        ene0 = h0
        ...
        ene1 = jnp.sum(gfA * h1) + jnp.sum(
                gfB * h1
        )

        f_up = jnp.einsum(
            "gij,jk->gik", chol, gfA.T, optimize="optimal"
        )
        f_dn = jnp.einsum(
            "gij,jk->gik", chol, gfB.T, optimize="optimal"
        )
    ...
    return jnp.sum(Eloc1 * ovlp1 * wigner)/totalOvlp # + Eloc2 * ovlp2 * wigner )/totalOvlp).real
