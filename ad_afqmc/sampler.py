...
def _block_scan(prop_data, _x, ham_data, propagator, trial, wave_data):
  ...
  _step_scan_wrapper = lambda x, y: _step_scan(x, y, ham_data, propagator, trial, wave_data)
...
def _ad_block(prop_data, ham_data, propagator, trial, wave_data):
  _sr_block_scan_wrapper = lambda x, y: _sr_block_scan(x, y, ham_data, propagator, trial, wave_data)
...
def propagate_phaseless_ad(ham, ham_data, coupling, observable_op, propagator, prop_data, trial, wave_data):
...
@partial(jit, static_argnums=(0,4,6))
def propagate_phaseless_ad_nosr(ham, ham_data, coupling, observable_op, propagator, prop_data, trial, wave_data):
  ham_data['h1'] = ham_data['h1'] + coupling * observable_op
  wave_data = trial.optimize_orbs(ham_data, wave_data)
  ham_data = ham.rot_orbs(ham_data, wave_data)
  ham_data = ham.rot_ham(ham_data, wave_data)
...
def propagate_phaseless_ad_nosr_norot(ham, ham_data, coupling, observable_op, propagator, prop_data, trial, wave_data):
  ...
  prop_data['n_killed_walkers'] = 0
  prop_data['pop_control_ene_shift'] = prop_data['e_estimate']
  prop_data, (block_energy, block_weight) = lax.scan(checkpoint(
...
def propagate_phaseless(ham, ham_data, propagator, prop_data, trial, wave_data):
  def _sr_block_scan_wrapper(x, y): return _sr_block_scan(x, y, ham_data, propagator, trial, wave_data)
  prop_data['overlaps'] = trial.calc_overlap_vmap(prop_data['walkers'], wave_data)
  prop_data['n_killed_walkers'] = 0
  prop_data['pop_control_ene_shift'] = prop_data['e_estimate']
  prop_data, (block_energy, block_weight) = lax.scan(_sr_block_scan_wrapper, prop_data, None, length=propagator.n_sr_blocks)
