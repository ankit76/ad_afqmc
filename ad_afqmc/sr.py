...
def stochastic_reconfiguration_mpi_uhf(walkers, weights, zeta, comm):
  ...
  weights = np.array(weights)
  walkers_new_up = 0. * walkers[0]
  walkers_new_dn = 0. * walkers[1]
  weights_new = 0. * weights
  ...
  if rank == 0:
    global_buffer_walkers_up = np.zeros(
        (nwalkers * size, walkers[0].shape[1], walkers[0].shape[2]), dtype=walkers[0]. dtype)
    global_buffer_walkers_new_up = np.zeros(
        (nwalkers * size, walkers[0].shape[1], walkers[0].shape[2]),               dtype=walkers[0].dtype)
    global_buffer_walkers_dn = np.zeros(
        (nwalkers * size, walkers[1].shape[1], walkers[1].shape[2]), dtype=walkers[0]. dtype)
  ...
  if rank == 0:
    ...
    global_buffer_weights_new = (
        np.ones(nwalkers * size) * average_weight).astype(weights.dtype)
    for i in range(nwalkers * size):
      z = (i + zeta) / nwalkers / size
      new_i = np.searchsorted(cumulative_weights, z * total_weight)
