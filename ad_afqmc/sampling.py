...
class sampler:
    ...
    def _block_scan_with_spin_obs(
        ...
        """Block scan function. Propagation, energy, and spin observables calculations."""
        prop_data["key"], subkey = random.split(prop_data["key"])
        fields = random.normal(
            subkey,
            shape=(
        ...
        # spin average <S> = [<Sx>, <Sy>, <Sz>].
        spin_avg_samples = jnp.real(
            trial.calc_spin_average(prop_data["walkers"], wave_data)
        )

        # spin square
        energy_samples = jnp.real(
            trial.calc_spin_square(prop_data["walkers"], wave_data)
        )

        block_weight = jnp.sum(prop_data["weights"])
        block_energy = jnp.sum(energy_samples * prop_data["weights"]) / block_weight
        block_spin_avg = jnp.sum(spin_avg_samples * prop_data["weights"]) / block_weight
        block_spin_sq = jnp.sum(spin_sq_samples * prop_data["weights"]) / block_weight
        prop_data["pop_control_ene_shift"] = (
            0.9 * prop_data["pop_control_ene_shift"] + 0.1 * block_energy
        )
        return prop_data, (block_energy, block_spin_avg, block_spin_sq, block_weight)
    ...
    @partial(jit, static_argnums=(0, 4, 5))
    def _sr_block_scan_with_spin_obs(
        self,
        prop_data: dict,
        _x: Any,
        ham_data: dict,
        ...
        _block_scan_wrapper = lambda x, y: self._block_scan_with_spin_obs(
            x, y, ham_data, prop, trial, wave_data
        )
        prop_data, (block_energy, block_spin_avg, block_spin_sq, block_weight) = lax.scan(
            _block_scan_wrapper, prop_data, None, length=self.n_ene_blocks
        )
        prop_data = prop.stochastic_reconfiguration_local(prop_data)
        prop_data["overlaps"] = trial.calc_overlap(prop_data["walkers"], wave_data)
    ...
    @partial(jit, static_argnums=(0, 1, 3, 5))
    def propagate_phaseless_with_spin_obs(
        self,
        ham: hamiltonian,
        ham_data: dict,
        prop: propagator,
        ...
        def _sr_block_scan_wrapper(x, y):
            return self._sr_block_scan_with_spin_obs(x, y, ham_data, prop, trial, wave_data)
...
class sampler_mixed(sampler):
    ...
    def propagate_phaseless(
        ...
        # Compute the average rdm1 over 1 sampling block.
        prop_data["block_rdm1"] /= jnp.sum(block_weight)
