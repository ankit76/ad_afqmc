...
ham_data_g = {}
ham_data_g["ene0"] = ham_data_u["ene0"]
ham_data_g["h0"] = ham_data_u["h0"]
ham_data_g["h1"] = jsp.linalg.block_diag(*ham_data_u["h1"])
ham_data_g["rot_h1"] = jsp.linalg.block_diag(*ham_data_u["rot_h1"])

rot_chol = np.zeros((nchol, nocc, 2*norb))
for i in range(nchol):
    rot_chol[i] = jsp.linalg.block_diag(
        ham_data_u["rot_chol"][0][i], 
        ham_data_u["rot_chol"][1][i]
    )

chol = np.zeros((nchol, 2*norb, 2*norb))
for i in range(nchol):
    chol_i = ham_data_u["chol"][i].reshape((norb, norb))
    chol[i] = jsp.linalg.block_diag(chol_i, chol_i)

ham_data_g["chol"] = chol
ham_data_g["rot_chol"] = rot_chol

# For legacy code.
ham_data_g_legacy = {}
ham_data_g_legacy["ene0"] = ham_data_u["ene0"]
ham_data_g_legacy["h0"] = ham_data_u["h0"]
ham_data_g_legacy["h1"] = ham_data_u["h1"]
ham_data_g_legacy["rot_h1"] = ham_data_g["rot_h1"]
ham_data_g_legacy["chol"] = ham_data_u["chol"]
ham_data_g_legacy["rot_chol"] = ham_data_g["rot_chol"]


# -----------------------------------------------------------------------------
# UHF wavefunction for CPMC.
uhf_cpmc = wavefunctions.uhf_cpmc(norb, nelec_sp) # nelec_sp = (3, 2) from above.
uhf_cpmc_legacy = wavefunctions_legacy.uhf_cpmc(norb, nelec_sp)

# -----------------------------------------------------------------------------
# GHF wavefunction for CPMC.
ghf_cpmc = wavefunctions.ghf_cpmc(norb, nelec_sp) # nelec_sp = (3, 2) from above.
ghf_cpmc_legacy = wavefunctions_legacy.ghf_cpmc(norb, nelec_sp)
...
def test_ghf_force_bias():
    ...
    force_bias_legacy = ghf_legacy._calc_force_bias(
        walker_up, walker_dn, ham_data_g_legacy, wave_data_g
    )
...
def test_ghf_force_bias_batch():
    ...
    force_bias_legacy = ghf_legacy.calc_force_bias(
        walkers_u, ham_data_g_legacy, wave_data_g
    )
...
def test_ghf_energy():
    ...
    energy_legacy = ghf_legacy._calc_energy(
        walker_up, walker_dn, ham_data_g_legacy, wave_data_g
    )
...
def test_ghf_build_measurement_intermediates():
    _ham_data_g = ham_data_g.copy()
    _ham_data_g = ghf._build_measurement_intermediates(_ham_data_g, _wave_data_g)

    _ham_data_u = ham_data_u.copy()
    _ham_data_u = uhf._build_measurement_intermediates(_ham_data_u, wave_data_u)
    
    assert _ham_data_g["rot_h1"].shape == (nocc, 2*norb)
    assert _ham_data_g["rot_chol"].shape == (nchol, nocc, 2*norb)

    np.testing.assert_allclose(
        _ham_data_u["rot_h1"][0],
        _ham_data_g["rot_h1"][: nelec_sp[0], : norb]
    )
    np.testing.assert_allclose(
        _ham_data_u["rot_h1"][1],
        _ham_data_g["rot_h1"][nelec_sp[0] :, norb :]
    )
    np.testing.assert_allclose(
        _ham_data_u["rot_chol"][0],
        _ham_data_g["rot_chol"][:, : nelec_sp[0], : norb]
    )
    np.testing.assert_allclose(
        _ham_data_u["rot_chol"][1],
        _ham_data_g["rot_chol"][:, nelec_sp[0] :, norb :]
    )
...
def test_ghf_cpmc_energy():
    energy = ghf_cpmc._calc_energy(walker_up, walker_dn, ham_data_g, _wave_data_g)
    energy_ref = uhf_cpmc._calc_energy(walker_up, walker_dn, ham_data_u, wave_data_u)

    np.testing.assert_allclose(energy, energy_ref)


def test_ghf_cpmc_energy_batch():
    energy = ghf_cpmc.calc_energy(walkers_u, ham_data_g, _wave_data_g)
    energy_ref = uhf_cpmc.calc_energy(walkers_u, ham_data_u, wave_data_u)

    np.testing.assert_allclose(energy, energy_ref)


def test_ghf_cpmc_energy_restricted():
    energy = ghf_cpmc._calc_energy_restricted(walker_g, ham_data_g, _wave_data_g)
    energy_ref = uhf_cpmc._calc_energy(walker_up, walker_dn, ham_data_u, wave_data_u)

    np.testing.assert_allclose(energy, energy_ref)


def test_ghf_cpmc_energy_restricted_batch():
    energy = ghf_cpmc.calc_energy(walkers_g, ham_data_g, _wave_data_g)
    energy_ref = uhf_cpmc.calc_energy(walkers_u, ham_data_u, wave_data_u)
...
if __name__ == "__main__":
    ...
    test_ghf_energy()
    test_ghf_rdm1()
    test_ghf_build_measurement_intermediates()
    ...
    test_ghf_cpmc_energy()
    test_ghf_cpmc_energy_restricted()
    ...
    test_ghf_cpmc_energy_batch()
    test_ghf_cpmc_energy_restricted_batch()
